...
import app
import pytest
from app import app, database, Totals, MAX_CLICK_RATE
...
def test_get_totals_should_return_incremented_click_count(client):
    start_click_count = int(json.loads(client.get('/totals?click_count=0').data)["click_count"])
    rv = client.get('/totals?click_count=10')
    data = json.loads(rv.data)
    assert data['click_count'] == start_click_count + 10
